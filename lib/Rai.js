...
export default function Rai(hostUrl) {
  this.error = function (error) {
    throw new Error(error);
  };


  this.rpc = function (request, asyncCallback) {
    try {
      var url = document.createElement('a');
      if (typeof hostUrl === 'undefined') { url.href = 'http://localhost'; } // if url is not set, use default to localhost
      else if (!hostUrl.startsWith('http')) { url.href = `http://${hostUrl.split('/').reverse()[0]}`; } // local files are not supported; default protocol = HTTP
      else { url.href = hostUrl; }

      if (url.port == '') { url.port = '7076'; } // default port 7076
    } catch (e) {
      if (e instanceof ReferenceError) {
        if (typeof hostUrl === 'undefined') { var url = 'http://localhost:7076'; } else { var url = hostUrl; }
