...
(function (nacl) {
  ...
  let randombytes = function (/* x, n */) { throw new Error('no PRNG'); };
  ...
  function Ch(x, y, z) {
    let h = (x.hi & y.hi) ^ (~x.hi & z.hi),
      l = (x.lo & y.lo) ^ (~x.lo & z.lo);
    return new u64(h, l);
  }

  function Maj(x, y, z) {
    let h = (x.hi & y.hi) ^ (x.hi & z.hi) ^ (y.hi & z.hi),
  ...
  function crypto_sign(sm, m, n, sk) {
    ...
    let context = blake2bInit(64, null);
    blake2bUpdate(context, sk);
    d = blake2bFinal(context);
    d[0] &= 248;
    d[31] &= 127;
